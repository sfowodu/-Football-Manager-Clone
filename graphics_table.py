...
def createrichladder(data, leaguecolor, teamcolor, userteam, gameweek):
    ...
    for team in new_dictionary:
        ...
        position += 1
        if team == userteam:
            table.add_row(str(position), str(data[team]["name"]), str(gameweek), str(data[team]["won"]), str(data[team]["drawn"]), str(data[team]["lost"]), str(data[team]["goals"]), str(data[team]["score"]), style=teamcolor)
        else:    
            table.add_row(str(position), str(data[team]["name"]), str(gameweek), str(data[team]["won"]), str(data[team]["drawn"]), str(data[team]["lost"]), str(data[team]["goals"]), str(data[team]["score"]))
